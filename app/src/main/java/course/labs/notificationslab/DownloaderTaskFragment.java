...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Integer> friendResIDS = getArguments().getIntegerArrayList(MainActivity.TAG_FRIEND_RES_IDS);
		
		// DONE: Start the DownloaderTask
		downloader.execute(friendResIDS.toArray(new Integer[friendResIDS.size()]));
	...
	public class DownloaderTask extends AsyncTask<Integer, Integer, String[]> {
		...
		@Override
		protected String[] doInBackground(Integer[] friendResIDs) {
			return downloadTweets(friendResIDs);
		}
	...
	private void notify(final boolean success) {
		...
		// Set up the intent with debugging so we can trace its resolution
		Intent intentDataRefresh = new Intent(MainActivity.DATA_REFRESHED_ACTION);
		intentDataRefresh.setFlags(Intent.FLAG_DEBUG_LOG_RESOLUTION);

		mContext.sendOrderedBroadcast(intentDataRefresh, null,
				new BroadcastReceiver() {
					...
					public void onReceive(Context context, Intent intent) {
						...
						if (getResultCode() != MainActivity.IS_ALIVE) {
							...
							PendingIntent restartMainActivityPendingIntent = PendingIntent.getActivity(context, 0, restartMainActivityIntent, PendingIntent.FLAG_UPDATE_CURRENT);
